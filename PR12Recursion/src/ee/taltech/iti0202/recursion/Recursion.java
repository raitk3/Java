...
//        return word.substring(word.indexOf("("), word.lastIndexOf(")") + 1);
        ...
        if (!word.substring(0, 1).equals("(") && !word.substring(word.length() - 1).equals(")")) {
            return parentheses(word.substring(1, word.length() - 1));
        }
        if (!word.substring(0, 1).equals("(") && word.substring(word.length() - 1).equals(")")) {
    ...
    public static String removeDuplicates(String word) {
        ...
        String restOfTheWord = removeDuplicates(word.substring(1));
        if (firstLetter.equals(restOfTheWord.substring(0, 1))) {
    ...
    public static String pidginfy(String word) {
        List<String> allowedLetters = Arrays.asList("a", "e", "i", "o", "u", "h", "k", "l", "m", "n", "p", "w",
            "r", "'", " ", "ā", "ō", "ū", ".", ",", "!", "?");
        if (word.length() <= 1 && allowedLetters.contains(word)) {
            return word;
        }
        if (word.length() <= 1 && !allowedLetters.contains(word)) {
            return "";
        }
        if (!allowedLetters.contains(word.substring(0, 1).toLowerCase())) {
