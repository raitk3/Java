...
public class Main {
    public static void main(String[] args) {
        ...
        Person juulius = new Person("Juulius", "Tipikas", 19, Person.Gender.MALE, 0);
        ...
        exceptionThrown = false;
        try {
            peepCreditCard.deposit(BigDecimal.TEN);
        } catch (TransactionException e) {
            exceptionThrown = true;
        }
        exceptionThrown = false;
        try {
            peepCreditCard.withdraw(BigDecimal.valueOf(11));
        } catch (TransactionException e) {
            exceptionThrown = true;
        }
        System.out.println(peepCreditCard.getBalance());
        System.out.println(peepCreditCard.getDebt());
        System.out.println(SEB.customers);
        BankCard juuliusCreditCard = BankCard.createCard(BankCard.CardType.CREDIT, LHV, juulius);
        BankCard peepCreditCard1 = BankCard.createCard(BankCard.CardType.CREDIT, LHV, peep);
        try {
            peepCreditCard1.deposit(BigDecimal.valueOf(223));
        } catch (TransactionException e) {
            exceptionThrown = true;
        }
        try {
            peepCreditCard1.withdraw(BigDecimal.valueOf(546484));
        } catch (TransactionException e) {}
        System.out.println(LHV.customers);
        // ...
